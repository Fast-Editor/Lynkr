...
Currently configured project: `claude-code` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/claude-code/SETUP.md` to fill in
   `data/claude-code/INFO.md`.
...
`--project-id claude-code` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/claude-code/`.
